...
AGENT_SUMMARY_URL = os.getenv("AGENT_SUMMARY_URL", "").strip()
AGENT_SUMMARY_TOKEN = os.getenv("AGENT_SUMMARY_TOKEN", "").strip()
AGENT_SUMMARY_REQUIRED = os.getenv("AGENT_SUMMARY_REQUIRED", "1").strip() == "1"
AGENT_RETRY_INTERVAL_SEC = int(os.getenv("AGENT_RETRY_INTERVAL_SEC", "20"))
AGENT_RETRY_MAX_ATTEMPTS = int(os.getenv("AGENT_RETRY_MAX_ATTEMPTS", "0"))
...
def parse_agent_summary_response(data: Any) -> Optional[Tuple[str, str]]:
    if isinstance(data, dict):
        if isinstance(data.get("summary"), str) and isinstance(data.get("analysis"), str):
            summary = data.get("summary", "").strip()
            analysis = data.get("analysis", "").strip()
            if summary and analysis:
                return truncate(summary, 500), truncate(analysis, 300)
...
async def agent_refine(session: aiohttp.ClientSession, title_zh: str, body_zh: str) -> Optional[Tuple[str, str]]:
    if not AGENT_SUMMARY_URL:
        if AGENT_SUMMARY_REQUIRED:
            return None
        return local_fallback_summary(body_zh, title_zh)

    prompt = (
        "请基于以下公告内容输出中文 JSON："
        '{"summary":"80-220字，提炼关键事实","analysis":"50-180字，给出影响分析"}。'
        "不要把“地区不可用”“一般性公告”放在前面；除非直接影响参与资格。"
    )
    payload = {
        "title_zh": title_zh,
        "body_zh": truncate(body_zh, 6000),
        "prompt": prompt,
        "output_format": {"summary": "string", "analysis": "string"},
    }
    headers = {"Content-Type": "application/json"}
    if AGENT_SUMMARY_TOKEN:
        headers["Authorization"] = f"Bearer {AGENT_SUMMARY_TOKEN}"
    ...
    try:
        async with session.post(
            ...
            if resp.status != 200:
                ...
                return local_fallback_summary(body_zh, title_zh)
            data = await resp.json(content_type=None)
            parsed = parse_agent_summary_response(data)
            if parsed:
                return parsed
    except Exception:
        if AGENT_SUMMARY_REQUIRED:
...
async def summarize_and_push_with_retry(
    title_en: str,
    title_zh: str,
    publish_ts: int,
    article_url: str,
    ...
    while True:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                refined = await agent_refine(session, title_zh, body_zh)
                if refined:
                    summary_zh, analysis_zh = refined
                    ...
                    print(f"[OK] 已推送飞书(尝试{attempt}): {title_en}")
                    return
        except Exception as e:
            print(f"[WARN] 后台总结/推送异常(尝试{attempt}): {title_en}, err={e}")

        if AGENT_RETRY_MAX_ATTEMPTS > 0 and attempt >= AGENT_RETRY_MAX_ATTEMPTS:
            print(f"[ERROR] 超过最大重试次数，放弃推送: {title_en}")
            return

        print(f"[WARN] agent未返回有效总结，{AGENT_RETRY_INTERVAL_SEC}s后重试: {title_en}")
        await asyncio.sleep(max(1, AGENT_RETRY_INTERVAL_SEC))
